  ...
  for (const [userId, userTasks] of Object.entries(tasksByUser)) {
    ...
    const taskListHtml = userTasks
      .map((t) => {
        ...
        return `<li style="margin-bottom:8px">
          <strong>${t.type}: ${t.title}</strong> — ${contactName}
          <br><span style="color:#666;font-size:13px">Due: ${tomorrowStr}</span>
        </li>`;
      })
      .join("");
    ...
    const html = `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
        <h2 style="color:#333">Reminder: ${userTasks.length} task${userTasks.length !== 1 ? "s" : ""} due tomorrow</h2>
        <ul style="padding:0;list-style:none">${taskListHtml}</ul>
        <p style="margin-top:24px">
          <a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard/tasks" style="background:#3b82f6;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;display:inline-block">View Tasks</a>
        </p>
      </div>
    `;

    await sendEmail(user.email, `Reminder: ${userTasks.length} task${userTasks.length !== 1 ? "s" : ""} due tomorrow`, html);
    sent++;
  }
